...
export default function Home() {
  ...
  const connectWallet = async () => {
    if (!(window as any).ethereum) return toast.error("MetaMask not found!");
    try {
  ...
  const findCheapest = async () => {
    ...
    try {
      ...
      setCheapest(best);
      toast.dismiss();
      toast.success(`Cheapest: ${best.chain.name}, Fee: ${ethers.utils.formatEther(best.estimatedCost.toString())} ${best.chain.nativeSymbol}`);
    } finally {
      setBusy(false);
    }
  };
  ...
  const switchToChain = async (chainHex: string) => {
    try {
      await (window as any).ethereum.request({ method: "wallet_switchEthereumChain", params: [{ chainId: chainHex }] });
      return true;
    } catch (err: any) {
      if (err.code === 4902) toast.error("Chain not found in wallet. Add it manually.");
      console.error(err);
      return false;
    }
  };
  ...
  const handleDonate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.dismiss();
      toast.success(`Donation sent on ${cheapest.chain.name}! TxHash: ${tx.hash}`);
    } catch (err) {
      console.error(err);
      toast.dismiss();
       ...
       <div className="w-full max-w-sm bg-white border border-gray-200 shadow-xl rounded-xl p-6">
          ...
          {cheapest && (
            <div className="mb-4 p-3 border border-gray-100 rounded-md bg-gray-50 text-sm text-gray-700">
              <p><strong>Cheapest:</strong> {cheapest.chain.name}</p>
              <p>
                <strong>Fee:</strong>{" "}
          ...
          <form onSubmit={handleDonate} className="space-y-4">
            ...
            <input
              type="number"
              placeholder={`Amount (${cheapest ? cheapest.chain.nativeSymbol : "XAI"})`}
              value={amount}
